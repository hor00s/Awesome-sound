...
class TestPlayer(unittest.TestCase):
    def setUp(self) -> None:
        other_list = ['1', '2', '4']
        self.songs = ['Astronaut', 'Tones And I', 'Lose yourself',
                      'Sing for the momment', 'Audacity']
        self.song = Disk(self.songs)
        self.song2 = Disk(other_list)
    ...
    def test_current_song(self):
        self.assertEqual(self.song.song_mp3, self.songs[0])

    def test_next(self):
        self.assertEqual(self.song.next(), self.songs[1])
        self.assertEqual(self.song.next(), self.songs[2])

        self.assertEqual(self.song2.song_mp3, self.song.song_mp3)
